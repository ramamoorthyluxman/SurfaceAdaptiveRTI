...
RAT = false;            %  true -> rational,  false -> standard
...
toto = gax();
tmp =  Images_Acq(:,1);
tmp = process_threshold(tmp, 0, 1);
[r, rx, ry] = toto.distanceMatrix2d(X, Y);
B = toto.rbf(r,s);
a = rbfx.solve(B, tmp, 0, true);
[re, rx, ry] = toto.distanceMatrix2d(X, Y, x, y);
H = toto.rbf(re,s);
ra = H*a;
%ra = rbfRational.interpolate2d(toto,s,double(tmp),double(X), double(x), double(Y), double(y));
% figure;
% imagesc(reshape(ra, m, n)); colormap gray;
 
% ra = rbfRational.interpolate2d(toto,8,f,xc,x,yc,y);
figure;
tri = delaunay(x,y);
trisurf(tri,x,y,ra);
view(150,19);
